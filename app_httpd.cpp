...
SoundData *data = new OneChannelSoundData((int16_t*)translator16bit_raw, translator16bit_raw_len / 2);
SoundData *piano_data = new OneChannelSoundData((int16_t*) piano16bit_raw, piano16bit_raw_len / 2);
...
esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    if (res == ESP_OK) {
      res = httpd_resp_send_chunk(req, _STREAM_BOUNDARY, strlen(_STREAM_BOUNDARY)); //NO stream
    }
    if (res == ESP_OK) {
      size_t hlen = snprintf((char *)part_buf, 64, _STREAM_PART, _jpg_buf_len);
      res = httpd_resp_send_chunk(req, (const char *)part_buf, hlen);// No format
    }
    if (res == ESP_OK) {
...
esp_err_t tts_handler(httpd_req_t *req) {
  char*  buf;
  size_t buf_len;
  char variable[32] = {0,};
  char value[32] = {0,};
  ...
  if (val == "s") {
    ...
    Serial.println("HAY INFORMACIÓN: "); Serial.print(a2dp_source.has_sound_data());
    delay(100);
    
    
     
  } else if (val == "p") {
    a2dp_source.write_data(piano_data);
